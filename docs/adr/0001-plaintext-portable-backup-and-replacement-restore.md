...
Backup files remain plaintext. The export flow permanently explains that anyone with the file can read its contents and requires confirmation on first use. This favors dependable recovery and portability for an offline-first spending tool over password and key-recovery complexity.
