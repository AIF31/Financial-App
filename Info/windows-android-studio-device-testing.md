# Windows, Android Studio, and physical-device testing
...
2. Use a representative physical Android device through Windows ADB for final acceptance testing.
...
## Connect a physical device through Windows ADB

Enable Developer options and USB debugging on the device, connect it by USB, accept the device's RSA prompt, and run:
...
The selected device must appear with state `device`. `offline` normally requires reconnecting the cable, restarting USB debugging, or rebooting the device. An empty device list means Windows ADB cannot currently see a device; Android Studio cannot compensate for that state.
...
The release verification report records which of these checks passed and which were omitted.
...
- Build Analyzer output is useful for future build-performance regressions; record any release-blocking findings in the verification report.
